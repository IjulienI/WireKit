...
#include "Slate/WireKitDetailsMutliColumn.h"
...
TSharedRef<SWidget> SWireKitDetailsMutliColumn::GenerateWidgetForColumn(const FName& InColumnName)
{
	if (InColumnName == TEXT("OutputName"))
	{
		return SNew(STextBlock)
			.Text_Lambda([this]()
			{
				return FText::FromName(Connection->OutputName);
			});
	}
	if (InColumnName == TEXT("TargetEntity"))
	{
		return SNew(STextBlock)
			.Text_Lambda([this]()
			{
				return FText::FromName(Connection->TargetEntity);
			});
	}
	if (InColumnName == TEXT("TargetInput"))
	{
		return SNew(STextBlock)
			.Text_Lambda([this]()
			{
				return FText::FromName(Connection->TargetInput);
			});
	}
	if (InColumnName == TEXT("Parameter"))
	{
		return SNew(STextBlock)
			.Text_Lambda([this]()
			{
				return FText::FromString(Connection->Parameter);
			});
	}
	if (InColumnName == TEXT("Delay"))
	{
		return SNew(STextBlock)
			.Text_Lambda([this]()
			{
				return FText::Format(LOCTEXT("DelayColumnFormat", "{0} s"), Connection->Delay);
			});
	}
	if (InColumnName == TEXT("bOnlyOnce"))
	{
		return SNew(SCheckBox)
			...
			{
				return Connection->bOnlyOnce ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
			});
